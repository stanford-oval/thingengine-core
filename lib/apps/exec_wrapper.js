...
const Compiler = ThingTalk.Compiler;
...
module.exports = class ExecWrapper extends ExecEnvironment {
    ...
    async invokeRule(program) {
        try {
            let ast = await Grammar.parseAndTypecheck(program, this.engine.schemas, true);
            let icon = null;

            let name = await Describe.getProgramName(this.engine.platform.getCapability('gettext'), ast);
            let description = await Describe.describeProgram(this.engine.platform.getCapability('gettext'), ast);
            await this.engine.apps.loadOneApp(program, { $icon: icon, }, undefined, undefined, name, description, true);
        } catch (e) {
            return this._output.error(this.icon, e);
        }
    }
