...
module.exports = class Memory extends events.EventEmitter {
    ...
    getSchema(table, principal=null) {
        if (principal !== null)
            return Promise.reject(new Error('Remote tables are not supported'));
    ...
    get(table, version, cols, filters, aggregation) {
        return this._db.withClient((client) => {
            const command = this._genSQLCommand(table, version, cols, filters, aggregation);
    ...
    _genSQLCommand(table, version, cols, filters, aggregation) {
        let where = '';
        if (version) {
    ...
    _queryCommand(table, cols, where) {
        return `select ${cols && cols.length ? cols.join(', ') : '*'} from "${prefix+table}" ${where}`;
    }
